...
public class ModeSwitch: UIControl {
    ...
    public var isOn: Bool {
        didSet {
            if isOn == true {
                creationController!.sendSwitchRequest(onRecord: true)
            } else {
                creationController!.sendSwitchRequest(onRecord: false)
            }
        }
    }
    ...
    @IBInspectable  public var onTintColor: UIColor =  UIColor.black {
        didSet {
            self.setupUI()
        }
    }
    ...
    init(controller: CreationSuiteController, frame: CGRect, onRecord: Bool) {
        isOn = onRecord
...
extension ModeSwitch {
    ...
    fileprivate func animate(on:Bool? = nil) {
        ...
        UIView.animate(withDuration: self.animationDuration, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 0.5, options: [UIView.AnimationOptions.curveEaseOut, UIView.AnimationOptions.beginFromCurrentState, UIView.AnimationOptions.allowUserInteraction], animations: {
            self.setupViewsOnAction()
